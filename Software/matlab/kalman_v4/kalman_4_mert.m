...
K_array = zeros(3, 3, N-1);   %# Creates a 2X2XN tensor
...
F = [1 dt dt^2/2; 0 1 dt ; 0 0 1]; % System Model
B = [dt^3/6 dt*dt/2 dt]; % Input Model
...
H2 = [0 1 0]; %-> When gyrometer measurement is taken
...
P_plus = diag([R(1) R(2) 1]); % Starting covarience
...
%     disp(debugg)
    ...
    if(z(1,k-1) ~= z(1,k)) % If the position did not change
        ...
        P_plus = (eye(3)-K*H_current)*P_minus;
    else
        H_current = H2; 
        ...
        K = (P_minus*H_current')*pinv(H_current*P_minus*H_current'+R(2,2));
