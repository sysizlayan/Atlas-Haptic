...
extern bool isGyroReady;
extern int32_t g_i32gyro_readingZ_raw;

//extern uint32_t g_ui32previousMeasurementTime;
//extern uint32_t g_ui32deltaTforGyro;
extern float g_fgyroVelocity;
extern const float resolution;
#ifdef CALIBRATE_GYRO
    const float NUMBER_OF_BIAS_SAMPLES = 1000;
    float g_fBiasTotal = 0;
    float g_fgyroBias = 0;
    uint32_t g_ui32gyroSamplingPeriodTotal = 0;
    uint32_t g_ui32gyroSamplingPeriod = 0;
    bool isGyroCalibrated = false;
    uint32_t biasSampleCount = 0;
#endif

//
// Velocity and position variables, they will be used globally
//
extern encoderReader g_sQEI1;
extern int32_t g_i32direction;
...
//Simulation model
extern const float k_spring;
extern const float M_mass;
extern const float b_damper;
...
extern const float R_pedal;

extern uint32_t g_ui32measurementInstance;
...
void initMPU6050()
{
    ...
    i2cWrite(MPU6050_ADDRESS, MPU6050_O_PWR_MGMT_1, MPU6050_PWR_MGMT_1_DEVICE_RESET);
    delay(100);
    while (i2cRead(MPU6050_ADDRESS, MPU6050_O_PWR_MGMT_1) & MPU6050_PWR_MGMT_1_DEVICE_RESET);
    delay(100);
    ...
    i2cBuffer[0] = MPU6050_INT_PIN_CFG_INT_LEVEL | MPU6050_INT_PIN_CFG_INT_RD_CLEAR | MPU6050_INT_PIN_CFG_LATCH_INT_EN;
    ...
    ROM_GPIOPadConfigSet(GPIO_PORTE_BASE,GPIO_PIN_3,GPIO_STRENGTH_8MA,GPIO_PIN_TYPE_STD);
...
void GPIOEHandler6050(void)
{
    ...
    g_ui32measurementInstance = micros();
    ...
    g_i32direction = get_dir(&g_sQEI1);
    ///////////////////////////////////////////////
    ...
    if(isGyroCalibrated)
    {
        ...
        pedalLinearPosition = R_pedal * sinf(g_fposition_filtered_plus * DEGREE_TO_RADIAN);
        pedalLinearVelocity = R_pedal * cosf(g_fposition_filtered_plus * DEGREE_TO_RADIAN) * g_fgyroVelocity * DEGREE_TO_RADIAN;
        ...
        g_fMassPosition = g_fMassPosition + B_0_0 * pedalLinearPosition  + B_0_1 * pedalLinearVelocity;

        g_fMassVelocity =                   A_1_0 * g_fMassPosition_prev + A_1_1 * g_fMassVelocity_prev;
        g_fMassVelocity = g_fMassVelocity + B_1_0 * pedalLinearPosition  + B_1_1 * pedalLinearVelocity;
...
//        UARTwrite(filteredPositionPointer,4); //Transmit filtered position as byte stream
    ...
    else if(biasSampleCount<NUMBER_OF_BIAS_SAMPLES)
    {
        ...
        g_fBiasTotal += g_fgyroVelocity;
        if(biasSampleCount == NUMBER_OF_BIAS_SAMPLES-1) // Gyro calibration is done
        {
            g_fgyroBias = g_fBiasTotal/NUMBER_OF_BIAS_SAMPLES;
